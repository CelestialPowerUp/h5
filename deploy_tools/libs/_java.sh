...
#date | git stash
#git checkout $local_branch && echo_color "1. checkout done!" &&
#git pull github_core $local_branch:$local_branch  &&  echo_color "2. pull done!" &&
...
rsync -avc --progress   ROOT --rsh="sshpass -p $remote_pwd ssh -o StrictHostKeyChecking=no -l root"  $remote_ip:$remote_tomcat_path/webapps   &&

echo_color "8. startup remote tomecat .." &&
sshpass -p $remote_pwd ssh -o StrictHostKeyChecking=no  $remote_name_ip "$remote_tomcat_path/bin/startup.sh"    && 
